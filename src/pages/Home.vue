...
const filteredSorted = computed<Movie[]>(() => {
  ...
  const filtered = movies.value.filter((m) => {
    ...
    const byGenres =
      !selectedGenres.value.length || selectedGenres.value.every((g) => m.genres.includes(g))
...
function fromQuery() {
  ...
  selectedGenres.value =
    typeof qy.genres === 'string' && qy.genres
...
<template>
  <section class="relative min-h-screen overflow-hidden bg-slate-950 text-white">
    ...
    <div class="pointer-events-none absolute inset-0">
      <div
        class="absolute -top-24 -left-24 h-80 w-80 rounded-full bg-gradient-to-tr from-indigo-500/40 via-fuchsia-500/40 to-transparent blur-3xl animate-pulse"
      ></div>
      <div
        class="absolute -bottom-24 -right-24 h-96 w-96 rounded-full bg-gradient-to-tr from-cyan-400/40 via-emerald-400/40 to-transparent blur-3xl animate-ping"
      ></div>
      <div
        class="absolute inset-0 bg-[radial-gradient(circle_at_20%_10%,rgba(59,130,246,.15),transparent_40%),radial-gradient(circle_at_80%_30%,rgba(236,72,153,.12),transparent_35%),radial-gradient(circle_at_50%_90%,rgba(34,197,94,.10),transparent_35%)]"
      ></div>
    </div>
    ...
    <header class="relative mx-auto max-w-7xl px-4 py-6 flex items-center justify-between">
      <RouterLink to="/" class="text-2xl font-extrabold tracking-tight">
        <span
          class="bg-gradient-to-r from-blue-400 via-fuchsia-400 to-emerald-400 bg-clip-text text-transparent"
          >Кинотека</span
        >
      </RouterLink>
    ...
    <div class="relative mx-auto max-w-7xl px-4 pb-6 pt-8 md:pt-16">
      <div class="grid items-center gap-10 md:grid-cols-2">
        <div>
          <h1 class="text-4xl font-black leading-tight md:text-6xl">
            <span class="bg-gradient-to-r from-white to-slate-300 bg-clip-text text-transparent"
              >Онлайн‑кинотека</span
            >
          </h1>
          <p class="mt-4 text-lg text-slate-300">
            Умный поиск, фильтры по жанрам, типам, годам и рейтингу. Топ‑карточки с плавными
            анимациями.
          </p>
          <div class="mt-6 flex flex-wrap gap-3">
            <RouterLink
              to="/catalog"
              class="rounded-xl bg-white px-5 py-2.5 font-semibold text-slate-900 shadow hover:shadow-lg transition"
              >Перейти в каталог</RouterLink
            >
            <a
              href="#filters"
              class="rounded-xl border border-white/20 px-5 py-2.5 font-semibold text-white/90 hover:bg-white/10 transition"
              >Настроить фильтры</a
            >
          </div>
        </div>
        <div class="relative">
          <div
            class="absolute -inset-4 -z-10 rounded-3xl bg-gradient-to-tr from-blue-500/20 via-fuchsia-500/20 to-emerald-500/20 blur-2xl"
          ></div>
    ...
    <div id="filters" class="relative mx-auto max-w-7xl px-4 pb-8">
      <div
        class="sticky top-0 z-20 -mx-4 mb-6 border-y border-white/10 bg-slate-950/70 px-4 py-4 backdrop-blur"
      >
        <div class="grid grid-cols-1 gap-3 md:grid-cols-12">
            ...
            <button
              ...
              :class="
                selectedGenres.includes(g)
                  ...
                  : 'bg-slate-900/60 text-slate-300 ring-white/10 hover:ring-white/20'
              "
            >
              {{ g }}
            </button>
          ...
          <div class="md:col-span-2">
            <select
              v-model="actor"
              class="w-full rounded-xl bg-slate-900/60 px-3 py-2.5 ring-1 ring-white/10"
            >
          ...
          <div class="md:col-span-6 flex items-center gap-3">
            <div class="w-full">
              <label class="mb-1 block text-xs text-slate-400"
                >Год выпуска: {{ yearFrom ?? minYear }} — {{ yearTo ?? maxYear }}</label
              >
          ...
          <div class="md:col-span-4 flex items-center gap-3">
            <div class="w-full">
              <label class="mb-1 block text-xs text-slate-400"
                >Рейтинг: {{ ratingFrom.toFixed(1) }} — {{ ratingTo.toFixed(1) }}</label
              >
              <div class="flex items-center gap-2">
                <input
                  type="range"
          ...
          <div class="md:col-span-2 flex items-center gap-2">
            ...
            <button
              type="button"
              @click="clearFilters"
              class="rounded-xl px-3 py-2 text-sm text-slate-200 ring-1 ring-white/10 hover:bg-white/10"
            >
              Сброс
            </button>
          </div>
      ...
      <div v-if="!filteredSorted.length" class="mt-12 text-center text-slate-400">
        Ничего не найдено
      </div>
    </div>
  </section>
